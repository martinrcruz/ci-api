...
class TipoValor extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model('tipoValor_model');
    ...
    public function getTipoValor($id_producto)
    {
        if (true) {
            ...
            $id_producto ? $where = " AND ID_PRODUCTO=$id_producto" : $where = '';


            if ($query = $this->tipoValor_model->getTipoValor($where)) {
    ...
    public function getTipoValorById()
    {
        if (true) {
            ...
            if ($this->input->post('id_tipo_valor')) {
                $request->id = trim($this->security->xss_clean($this->input->post('id_tipo_valor', true)));
            ...
            $request->id ? $where = " AND ID_TIPO_VALOR=$request->id" : $where = '';
    ...
    public function insertTipoValor()
    {
        if (true) {
            ...
            if (!empty($this->input->post('valor'))) {
                $request->valor = $this->security->xss_clean($this->input->post('valor'));
            }
            ...
            if ($query = $this->tipoValor_model->insertTipoValor('tipo_valor', $datos)) {
                $response->proceso = 1;
                $response->id = $query;
                $response->data = $datos;
            } else {
    ...
    public function updateTipoValor()
    {
        if (true) {
            ...
            if (sizeof($response->errores) == 0) {
                if ($query = $this->tipoValor_model->updateTipoValor('tipo_valor', 'ID_TIPO_VALOR', $datos, $request->id)) {
    ...
    public function deleteTipoValor()
    {
        if (true) {
            ...
            if ($this->input->post('id_tipo_valor')) {
                $request->id_tipo_valor = $this->security->xss_clean($this->input->post('id_tipo_valor'));
            } else { //SI NO, ALMACENAMOS EL ERROR EN UN ARRAY PARA DEVOLVERLO COMO RESPUESTA.
                $response->errores[] = "Ocurrió un problema al obtener la solicitud";
            }

            $where = " AND ID_TIPO_VALOR=$request->id_tipo_valor";
            $itemEliminado = $this->tipoValor_model->getTipoValor($where);

            $response->data = $itemEliminado->result();

            //SI ES QUE NO HAY ERRORES, PROCEDEMOS A HACER LA PETICION MEDIANTE UN LLAMADO A LA FUNCION DEL MODELO.
            if (sizeof($response->errores) == 0) {
                if ($this->tipoValor_model->updateTipoValor("tipo_valor", "ID_TIPO_VALOR", array('FECHA_BAJA' => $fecha, "ESTADO" => 0), $request->id_tipo_valor)) {
                    //SI EL PROCESO ES EXITOSO, DEVOLVERA UN VALOR DENTRO DEL ARRAY DE RESPUESTA IGUAL A 1
                    $response->proceso = 1;
                }
            } else {
